        ...
        routes.forEach(route => {
            if (route.interceptors) {
                if (route.method) {
                    ...
                    // Match api, because of there is not `method`
                    router.all(route.path, this._getInterceptors(route.interceptors), this._getAction(route.action));
                }
            } else {
                if (route.method) {
                    router[route.method.toLowerCase()](route.path, this._getAction(route.action));
                } else {
                    router.all(route.path, this._getAction(route.action));
                }
            }
        });
